...
int main(int argc, char *argv[])
{
    ...
    int out_fd = open(data.output, O_CREAT | O_WRONLY | O_TRUNC, S_IREAD | S_IWRITE);
    // data.crashing_input[data.crashing_input_size] = 0x0;

    ssize_t s = data.crashing_input_size;
    ssize_t count = 0;
    while (count < data.crashing_input_size)
    {
        count += write(out_fd, data.crashing_input + count, s - count);
    }

    // int sent;
    // int count = 0;
    // while(sent = write(out_fd, data.crashing_input+count, data.crashing_input_size-count)){
    //     count += sent;
    // }
